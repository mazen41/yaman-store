...
/**
 * Edit Status Page
 * - This file handles EDITING existing statuses only.
 */
...
if (!$status_id || !$table_name || !in_array($table_name, $allowed_tables)) {
     $_SESSION['error_message'] = 'بيانات غير صالحة للوصول لهذه الصفحة.';
     header('Location: index.php');
     exit();
}

if (!hasPermission($user_id, 'statuses', 'edit')) {
    $_SESSION['error_message'] = 'ليس لديك صلاحية لتعديل الحالات.';
    header('Location: index.php');
    exit();
}

// Fetch the existing status data
try {
    $stmt = $db->prepare("SELECT * FROM `{$table_name}` WHERE id = ?");
    $stmt->execute([$status_id]);
    $status = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$status) {
        $_SESSION['error_message'] = 'الحالة غير موجودة.';
        header('Location: index.php');
        exit();
    }
} catch (PDOException $e) {
    $_SESSION['error_message'] = 'خطأ في استرجاع الحالة: ' . $e->getMessage();
    header('Location: index.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($status['status_key']) || empty($status['status_name_ar'])) {
        ...
        try {
            // Check for duplicates, EXCLUDING the current status's own ID
            $stmt = $db->prepare("SELECT id FROM `{$table_name}` WHERE status_key = ? AND id != ?");
            $stmt->execute([$status['status_key'], $status_id]);
            if ($stmt->fetch()) {
                $error_message = 'المعرّف (Key) مستخدم بالفعل في حالة أخرى. الرجاء اختيار معرّف آخر.';
            } else {
                // No duplicate found, proceed with update
                $stmt = $db->prepare("UPDATE `{$table_name}` SET status_key = ?, status_name_ar = ?, is_default = ? WHERE id = ?");
                $stmt->execute([$status['status_key'], $status['status_name_ar'], $status['is_default'], $status_id]);

                if ($status['is_default'] == 1) {
                    $reset_stmt = $db->prepare("UPDATE `{$table_name}` SET is_default = 0 WHERE id != ?");
                    $reset_stmt->execute([$status_id]);
                }
                
                $_SESSION['success_message'] = 'تم تحديث الحالة بنجاح.';
                header('Location: index.php');
                exit();
            }
        } catch (PDOException $e) {
...
<div dir="rtl">
    <div class="form-wrapper">
        ...
        <form method="POST" action="edit_status.php?id=<?php echo htmlspecialchars($status_id); ?>&table=<?php echo htmlspecialchars($table_name); ?>">
            <div class="form-group">
                <label for="status_key">المعرّف (Key)</label>
                <input type="text" id="status_key" name="status_key" class="form-control" value="<?php echo htmlspecialchars($status['status_key']); ?>" required>
                <small style="color: #6b7280;">أحرف إنجليزية صغيرة، أرقام، وشرطة سفلية (_) فقط.</small>
            </div>

            <div class="form-group">
                <label for="status_name_ar">الاسم بالعربي</label>
                <input type="text" id="status_name_ar" name="status_name_ar" class="form-control" value="<?php echo htmlspecialchars($status['status_name_ar']); ?>" required>
            </div>
            ...
            <div class="form-actions">
                ...
                <a href="index.php" class="btn btn-secondary"><i class="fas fa-times"></i> إلغاء</a>
            </div>
        </form>
    </div>
</div>
