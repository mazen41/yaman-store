...
// Check permission
if (!hasPermission($_SESSION['user_id'], 'customer_invoices', 'edit')) {
    $_SESSION['error_message'] = 'ليس لديك صلاحية للتعديل';
    header('Location: index.php');
    exit();
}

$page_title = 'تعديل فواتير العملاء';
$id = intval($_GET['id'] ?? 0);

// Fetch item
try {
    $stmt = $db->prepare("SELECT * FROM customer_invoices WHERE id = ?");
    $stmt->execute([$id]);
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8" dir="rtl">
    <div class="bg-white rounded-xl shadow-lg p-8">
        <h1 class="text-2xl font-bold text-gray-900 mb-6">تعديل فواتير العملاء</h1>
