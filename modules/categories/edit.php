...
try {
    ...
    if (!$category) {
        $error_message = 'الفئة غير موجودة.';
        // Consider redirecting or handling this gracefully
    }

    // Fetch main categories for the parent_id dropdown, excluding the current category and its sub-categories
    $stmt = $db->prepare("SELECT id, name FROM categories WHERE parent_id IS NULL AND is_active = 1 AND id != ? ORDER BY name");
    $stmt->execute([$category_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $can_edit && $category) {
    $name = trim($_POST['name']);
    $parent_id = $_POST['parent_id'] ?? null;
    $display_order = $_POST['display_order'] ?? 0;
    
    $current_image_url = $category['image_url']; // Keep existing image by default
    ...
    if (isset($_FILES['image_file']) && $_FILES['image_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($file_size > $max_file_size) {
            ...
            if (move_uploaded_file($file_tmp_name, $destination_path)) {
                $current_image_url = $base_image_url . $unique_filename; // Update to new image URL
    ...
    } elseif ($parent_id == $category_id) { // Parent cannot be itself
        $error_message = 'لا يمكن أن تكون الفئة أب لنفسها.';
    } elseif (empty($error_message)) { // Only proceed if no image upload errors
        try {
            $stmt = $db->prepare("UPDATE categories SET name = ?, parent_id = ?, display_order = ?, image_url = ?, updated_at = NOW() WHERE id = ?");
            $stmt->execute([
                $name,
                $parent_id === '' ? null : $parent_id,
                $display_order,
                $current_image_url, // Use the new URL if uploaded, else the old one
                $category_id
            ]);
            ...
            if ($new_image_uploaded && $category['image_url']) {
                $old_file_path = __DIR__ . '/../..' . $category['image_url'];
                if (file_exists($old_file_path) && is_file($old_file_path)) {
                    unlink($old_file_path); // Delete the actual file
                }
            }

            // Re-fetch category data to reflect changes
            $stmt = $db->prepare("SELECT * FROM categories WHERE id = ?");
            $stmt->execute([$category_id]);
            $category = $stmt->fetch(PDO::FETCH_ASSOC);

        } catch (PDOException $e) {
            $error_message = 'حدث خطأ أثناء تحديث الفئة: ' . $e->getMessage();
        }
    }
}
    ...
    <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow rounded-lg mb-6">
            <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                <h1 class="text-2xl font-bold text-gray-900">تعديل الفئة: <?php echo htmlspecialchars($category['name'] ?? 'غير موجودة'); ?></h1>
                <a href="index.php"
            ...
            <div class="p-6">
                ...
                <?php elseif (!$category): ?>
                     <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg mb-6 text-center">
                        ...
                        <p>الفئة المطلوبة غير موجودة.</p>
                    </div>
                <?php else: ?>
                    <!-- Add enctype for file uploads -->
                    <form action="edit.php?id=<?php echo htmlspecialchars($category_id); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                        <div>
                            ...
                            <select name="parent_id" id="parent_id"
                                ...
                                <?php foreach ($main_categories as $cat): ?>
                                    <option value="<?php echo $cat['id']; ?>"
                                        <?php if (($category['parent_id'] ?? '') == $cat['id']) echo 'selected'; ?>>
                        ...
                        <div>
                            ...
                            <input type="file" name="image_file" id="image_file" accept="image/*"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                            <p class="mt-2 text-sm text-gray-500">يمكنك رفع صورة جديدة (JPG, JPEG, PNG, GIF) بحد أقصى 5 ميجابايت. إذا لم ترفع صورة جديدة، ستبقى الصورة الحالية.</p>
                            <?php if ($category['image_url']): ?>
                                <div class="mt-4">
                                    <p class="text-sm text-gray-600 mb-2">الصورة الحالية:</p>
                                    <img src="<?php echo htmlspecialchars($category['image_url']); ?>" alt="صورة الفئة" class="h-24 w-24 object-cover rounded-lg border border-gray-200">
                                    <!-- Optionally, add a checkbox to remove the image without replacing it -->
                                    <!-- <div class="mt-2 flex items-center">
                                        <input type="checkbox" name="remove_image" id="remove_image" class="form-checkbox h-4 w-4 text-red-600">
                                        <label for="remove_image" class="mr-2 text-sm text-red-700">حذف الصورة الحالية</label>
                                    </div> -->
                                </div>
                            <?php else: ?>
                                <p class="mt-4 text-sm text-gray-600">لا توجد صورة حالية لهذه الفئة.</p>
                            <?php endif; ?>
                        </div>
