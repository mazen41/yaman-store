...
if (file_exists('../../includes/check_permissions.php')) {
    ...
    // Assuming 'categories' is the permission key for this module
    $can_view = hasPermission($_SESSION['user_id'], 'categories', 'view');
    $can_add = hasPermission($_SESSION['user_id'], 'categories', 'add');
    $can_edit = hasPermission($_SESSION['user_id'], 'categories', 'edit');
}

if (!$can_view) {
    $error_message = 'ليس لديك صلاحية لعرض قائمة الفئات.';
}
    ...
    if (!$can_edit) {
        ...
        try {
            if ($_GET['action'] == 'toggle_active') {
                ...
                $success_message = $new_status == 1 ? 'تم تفعيل الفئة بنجاح.' : 'تم تعطيل الفئة بنجاح.';
            } elseif ($_GET['action'] == 'delete') {
                // Soft delete: set is_active to 0
                $stmt = $db->prepare("UPDATE categories SET is_active = 0, updated_at = NOW() WHERE id = ?");
                $stmt->execute([$category_id]);
                $success_message = 'تم حذف الفئة بنجاح (أصبحت غير نشطة).';
            }
...
$filter_type = $_GET['filter_type'] ?? ''; // main, sub, all
...
$sort_column = $sort_options[$sort_by] ?? 'c.display_order';
...
// Apply type filter (main/sub)
if ($filter_type == 'main') {
    $where_clauses[] = "c.parent_id IS NULL";
} elseif ($filter_type == 'sub') {
    $where_clauses[] = "c.parent_id IS NOT NULL";
}
        ...
        <div class="bg-white shadow rounded-lg mb-6">
            <div class="px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="text-gray-600 mt-1">عرض، إضافة، وتعديل فئات المنتجات.</p>
                </div>
                <div class="mt-4 sm:mt-0 flex items-center space-x-3 space-x-reverse">
                    ...
                    <?php if ($can_add): ?>
                        <a href="add.php"
                            ...
                            <i class="fas fa-plus ml-2"></i> فئة جديدة
                        </a>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <form method="GET">
                ...
                <div class="px-6 py-4 border-b border-gray-200">
                    <div class="flex flex-wrap gap-4 items-end">
                        <div style="flex-grow: 1;">
                            <label for="search" class="block text-sm font-medium text-gray-700 mb-1">بحث (اسم الفئة)</label>
                            <input type="text" name="search" id="search" placeholder="ابحث باسم الفئة..."
                ...
                <div id="advancedFilters" class="px-6 py-4" style="display: <?php echo $advanced_filters_active ? 'block' : 'none'; ?>;">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 items-end">
                        ...
                        <div>
                            <label for="filter_type" class="block text-sm font-medium text-gray-700 mb-1">نوع الفئة</label>
                            <select name="filter_type" id="filter_type"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                                <option value="">الكل</option>
                                <option value="main" <?php if ($filter_type == 'main') echo 'selected'; ?>>فئة رئيسية</option>
                                <option value="sub" <?php if ($filter_type == 'sub') echo 'selected'; ?>>فئة فرعية</option>
                            </select>
                        </div>
                        <!-- Sort By -->
                        <div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">الصورة</th>
                            <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">اسم الفئة</th>
                            <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">النوع</th>
                        ...
                        <?php elseif (empty($categories)): ?>
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-gray-500"><i
                                        class="fas fa-folder text-4xl mb-4 text-gray-300"></i>
                                    <p>لا توجد فئات مطابقة للبحث أو الفلترة.</p>
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($categories as $category): ?>
                                <tr class="hover:bg-gray-50 <?php echo $category['is_active'] == 0 ? 'bg-red-50' : ''; ?>">
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900 text-center">
                                        <?php if ($category['image_url']): ?>
                                            <img src="<?php echo htmlspecialchars($category['image_url']); ?>" alt="<?php echo htmlspecialchars($category['name']); ?>" class="h-10 w-10 object-cover rounded-full mx-auto">
                                        <?php else: ?>
                                            <i class="fas fa-image text-gray-300 text-2xl"></i>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900 text-center">
                                        <div class="font-bold <?php echo $category['is_active'] == 0 ? 'text-red-600' : ''; ?>">
                                            <?php echo htmlspecialchars($category['name']); ?>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900 text-center">
                                        <?php if ($category['parent_id']): ?>
                                            <span class="px-2 py-1 text-xs font-semibold rounded-full bg-indigo-100 text-indigo-800">فرعية (<?php echo htmlspecialchars($category['parent_category_name']); ?>)</span>
                                        <?php else: ?>
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap text-sm font-medium text-center">
                                        <div class="flex justify-center items-center space-x-2 space-x-reverse">
                                            <a href="view.php?id=<?php echo $category['id']; ?>"
                                            ...
                                            <?php if ($can_edit): ?>
                                                ...
                                                <a href="?action=toggle_active&id=<?php echo $category['id']; ?>&status=<?php echo $category['is_active'] . '&' . http_build_query(array_filter($_GET, fn($k) => $k != 'action' && $k != 'id' && $k != 'status', ARRAY_FILTER_USE_KEY)); ?>"
                                                    class="<?php echo $category['is_active'] ? 'text-red-600 hover:text-red-900' : 'text-green-600 hover:text-green-900'; ?>"
                                                    title="<?php echo $category['is_active'] ? 'تعطيل' : 'تفعيل'; ?>"
                                                    onclick="return confirm('<?php echo $category['is_active'] ? 'هل أنت متأكد من تعطيل هذه الفئة؟' : 'هل أنت متأكد من تفعيل هذه الفئة؟'; ?>')">
                ...
                <div
                    ...
                    <div class="w-full sm:w-auto text-center sm:text-right mb-4 sm:mb-0">
                        <p class="text-sm text-gray-700">
                            ...
                            إلى <span
                                class="font-medium"><?php echo min($offset + $limit, $total_categories); ?></span>
                            من <span class="font-medium"><?php echo $total_categories; ?></span> نتيجة
                        </p>
                    </div>
