...
// Check permission
if (!hasPermission($_SESSION['user_id'], 'shop_coupons', 'edit')) {
    $_SESSION['error_message'] = 'ليس لديك صلاحية للتعديل';
    header('Location: index.php');
    exit();
}

$page_title    = 'تعديل كوبون متجر';
$id            = intval($_GET['id'] ?? 0);
$error_message = '';
...
try {
    ...
    if (!$item) {
        $_SESSION['error_message'] = 'الكوبون غير موجود';
        header('Location: index.php');
        exit();
    }
...
// Map DB discount_type (percentage/fixed) → form values (percent/amount)
$discount_type_form = ($item['discount_type'] ?? '') === 'fixed' ? 'amount' : 'percent';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            $_SESSION['success_message'] = 'تم تعديل الكوبون بنجاح';
            header('Location: index.php');
            exit();
        } catch (PDOException $e) {
            if ($e->getCode() == 23000) {
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8" dir="rtl">
    <div class="bg-white rounded-xl shadow-lg p-8">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-900">تعديل كوبون متجر</h1>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="code" value="<?php echo htmlspecialchars($form['code']); ?>"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 font-mono uppercase" required>
                </div>
